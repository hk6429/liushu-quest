...
const LSFlash = (() => {
  ...
  function buildDeck(level, focusIds = pendingFocus) {
    ...
    const deck = [...focusedWeak, ...dueWeak, ...focusedOther, ...dueOther].slice(0, ROUND);
...
<div class="card">
  ...
  <div id="flashArea"></div>
    ...
    el.querySelector('#flashStart').onclick = () => {
      ...
      st = { deck: buildDeck(level), i: 0, flipped: false, completed: false, lastGrowth: '' };
      pendingFocus = [];
  ...
  function show(area) {
    if (st.i >= st.deck.length) {
      const earned = st.completed ? [] : LSStore.completeSession('flash', { total: st.deck.length });
      st.completed = true;
