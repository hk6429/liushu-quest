...
- 輸入來源：本站原創故事、六書概念、歷史人物姓名、字例白話解說，以及「潑墨酒精暈染＋Q 版人物」和 picture3 暖米白 pastel 視覺設定。所有圖像生成均未傳入第三方參考圖；其後提供的遊戲畫面只用於大師對戰介面資訊層次討論，沒有存入或重製於本儲存庫。
...
- 視覺規格：picture3 暖米白、pastel 模組、細線外框、Q 版教學插畫；16:9；圖內不放漢字、古文字、字母、數字、標籤、logo 或浮水印。
- 原始 PNG：依 lane 保存在 `~/Downloads/liushu-picture3-source/lane-1/` 至 `lane-4/`，不納入 Git，避免儲存庫膨脹。
- 完整逐檔 ID、字、路徑、SHA-256、生成日期及原始檔位置見 `docs/char-image-provenance.json`；四批工作紀錄見 `docs/image-lane-1.md` 至 `docs/image-lane-4.md`。
